...
// get github registration token that allows to register new runner based on GH_PERSONAL_ACCESS_TOKEN github user api key
...
async function start(label) {
  ...
  // we better keep GH_PERSONAL_ACCESS_TOKEN here and do not pass it to AWS EC2 userscript so it will keep secret
  ...
  let last_error; // last error that ill be thrown in case all our attemps in instance creation will fails
  for (let spot of (core.getInput("aws-spot") == "true" ? [1, 0] : [0])) { // loop for spot/ondemand instances
    ...
    for (let ec2type of ec2types) { // iterate for provided instance types
      const setup_github_actions_runner = [
        ...
        `cd actions-runner`,
        // we can not place runner into AMI image since it is updated often and latest version in required to connect to github
        ...
        core.getInput("one-job") == "true" ? `su gh_runner -c "./run.sh --ephemeral"` : `su gh_runner -c "./run.sh"`,
        `su gh_runner -c "./config.sh remove --token ${reg_token}"`,
        `shutdown -h now` // in case we launch insance with InstanceInitiatedShutdownBehavior = "terminate" it will terminate instance here as well
      ];
      try {
        let params = {
    ...
    if (ec2id) break; // we already created instance and do not need to iterate these loops
  }
  if (last_error) {
  ...
  // wait untill instance will be found running before continuing (spot instance can be created but never run and will be in pending state untill termination)
...
// terminate (completely remove) AWS EC instances (normally one instance) with given tag label and also remove all Github actions runners (normally one runner) with that label
async function stop(label) {
  let last_error; // last error that will be thrown in case something will break here
  const ec2 = new AWS.EC2();
  ...
  if (instances) for (const reservation of instances.Reservations) {
  ...
  if (runners) for (runner of runners) {
    ...
    // retry removal up to 5 times (with 10 seconds delay) sincec Github can not remove runners still marked as active (with running job)
    for (let i=0; i < 5; i++) {
